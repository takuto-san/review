...
独立验证机械检查以及结构和上下文审查结果并生成PR评论候选。不得增加新关注点、修改文件或发布GitHub评论。

## 必需输入

必须提供收集后的上下文、Change Scope、完整审查计划、结构和上下文审查结果、完整的`review.mechanical` Artifact以及`REVIEW.md`。缺失时不得重建或猜测，应将相关前提标记为未完成。
...
3. 确认所有适用审查层均已完成。
4. 确认机械Artifact包含整体`result`、摘要以及每个已执行检查的记录。
5. 确认每项检查的`result`与退出码一致，并且只有全部检查通过时整体`result`才为`passed`。
...
返回一个具有`name: review.verification`和`metadata.schema: review/verification`的A2A兼容Artifact，并将以下负载放入`parts[0].data`：
