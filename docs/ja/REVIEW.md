...
適用するレビュー観点を変更がどの程度満たしているか、次の5段階で評価する。直感ではなく、具体的な実装、実行経路、テスト、仕様の証拠に基づいて段階を選択する。
...
最初の4段階はレビュー観点への適合度を表し、`not_assessable`は利用可能な証拠から適合度を判断できないことを表す。この尺度は、人間へ求める対応を表すものではない。`not_assessable`は「確認不可」と組み合わせ、評価可能な結果は以下の結果分類によって別途分類する。
