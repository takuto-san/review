...
収集済みコンテキスト、Change Scope、完全なレビュー計画、全レビュー結果、トップレベルの`checks`を含む完全な機械的レビューArtifact、`REVIEW.md`が必要です。不足する場合は再構築や推測をせず、該当する前提条件を未完了とします。
...
4. 静的解析とUnitテストが実行され、対象コマンドごとのトップレベルのチェック記録があるか確認する。未実行なら理由を確認する。
5. 機械的レビューの各`assessment.check_refs`が実在するチェックを参照し、結果、終了コード、要約が整合するか確認する。
